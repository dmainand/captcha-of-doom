...
class Enemy extends BaseEnemy {
    ...
    update(dt) {
        ...
        if (this.lastLarbinTime <= 0 && this.larbin_count < this.max_larbin){
            this.lastLarbinTime = 20;
    ...
    collect() {
        ...
        if (map.levels.length>1) {
            ...
            setTimeout(_ => {
                ...
                hE.innerHTML=`Vous êtes bien un humain !<br> Redirection vers votre page d'accès<br><br>`
                setTimeout(_=>{
                    window.parent.open("/accueil.html");
                    window.close()},2000);
            }, 200)
...
function ManyEnemies(position, count) {
    range(DIFFICULTY*5+5+count*50*Math.PI|0).map(_=> objects.push(new ([Enemy,FlyingEnemy][_%2])(position.add(urandom_vector().noz().scalar_multiply(20)),
                                                  urandom()*10)));
